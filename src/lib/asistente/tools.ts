...
export const ASISTENTE_TOOLS: Tool[] = [
  ...
  },
  {
    ...
    description: 'Busca un producto específico por nombre en TODO el catálogo de ventas de ConectOca (Producción) — no solo el top 15 más vendido. Usar para preguntas sobre un producto puntual, incluida su participación porcentual sobre el total (para eso, llamar también a obtener_informe_periodo del mismo rango y calcular el % vos mismo). Cada resultado trae "porLocalIdentificado" (cuánto pidió cada sucursal — La Reina, PV, PT, Bilbao) y "sinIdentificar". LIMITACIÓN REAL DEL DATO, no negociable al responder: ConectOca no tiene la sucursal identificada en la mayoría de los pedidos (típicamente ~80% del total cae en "sinIdentificar", no es que esa sucursal no haya pedido, es que ese pedido puntual no quedó con el dato de sucursal cargado). "porLocalIdentificado" SÍ es real — es cuánto pidió cada sucursal, no un total aparte ni algo desconectado de la venta. Nunca lo presentes como si cubriera el 100% de los pedidos: siempre mencioná también sinIdentificar para que quede claro que es la parte que sí se pudo identificar, no la única que existe.',
    input_schema: {
      type: 'object',
      properties: {
