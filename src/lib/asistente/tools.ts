...
export const ASISTENTE_TOOLS: Tool[] = [
  ...
  },
  {
    ...
    description: 'Busca un producto específico por nombre en TODO el catálogo de ventas de ConectOca (Producción) — no solo el top 15 más vendido. Usar para preguntas sobre un producto puntual, incluida su participación porcentual sobre el total (para eso, llamar también a obtener_informe_periodo del mismo rango y calcular el % vos mismo). Cada resultado trae "porLocalIdentificado" (desglose por sucursal — La Reina, PV, PT, Bilbao) y "sinIdentificar". LIMITACIÓN REAL DEL DATO, no negociable al responder: ConectOca no registra sucursal en la mayoría de los pedidos (típicamente ~80% del total cae en "sinIdentificar") — "porLocalIdentificado" viene de un campo de cliente que se usa para pedidos mayoristas/de reposición hacia las sucursales, NO es el ticket de venta al público. Nunca presentes porLocalIdentificado como si fuera el total real vendido en cada local: siempre mencioná también sinIdentificar y aclará que es un desglose parcial.',
    input_schema: {
      type: 'object',
      properties: {
