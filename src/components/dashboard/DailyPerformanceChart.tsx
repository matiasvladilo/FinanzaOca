  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h3 className="text-[14px] font-bold text-gray-900">Ventas vs Gastos — Por Mes</h3>
      ...
      {loading ? (
        <div className="h-[260px] bg-gray-100 rounded-xl animate-pulse" />
      ) : data.length === 0 ? (
        <div className="h-[260px] flex items-center justify-center">
          <p className="text-[12px] text-gray-400">Sin datos para el período seleccionado</p>
        </div>
      ) : (
        <ResponsiveContainer width="100%" height={260}>
          {chartType === 'bar' ? (
            <BarChart data={data} barCategoryGap="30%" barGap={2}>
              ...
              <Bar dataKey="gastos" name="Gastos" fill="#D1D5DB" radius={[4, 4, 0, 0]} />
            </BarChart>
          ) : (
            <LineChart data={data}>
              {commonAxis}
              ...
              <Line type="monotone" dataKey="gastos" name="Gastos" stroke="#D1D5DB" strokeWidth={2} dot={{ r: 3, fill: '#D1D5DB', stroke: '#fff', strokeWidth: 2 }} activeDot={{ r: 5 }} />
            </LineChart>
          )}
        </ResponsiveContainer>
      )}
