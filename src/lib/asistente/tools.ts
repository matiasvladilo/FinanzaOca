...
export const ASISTENTE_TOOLS: Tool[] = [
  ...
  },
  {
    ...
    description: 'Busca un producto específico por nombre en TODO el catálogo de ventas de ConectOca (Producción) — no solo el top 15 más vendido. Usar para preguntas sobre un producto puntual, incluida su participación porcentual sobre el total (para eso, llamar también a obtener_informe_periodo del mismo rango y calcular el % vos mismo). Cada resultado trae "porLocalIdentificado" (cuánto pidió cada sucursal — La Reina, PV, PT, Bilbao, resuelto por la cuenta de ConectOca que hizo cada pedido) y "sinIdentificar" (pedidos de canales que no son ninguna de las 4 sucursales, ej. despacho — normalmente muy chico o cero). porLocalIdentificado es un dato completo y real: si sinIdentificar sale en 0 (lo más común), decilo así de directo. Si sinIdentificar tiene algo, mencionalo de pasada, sin darle un peso que no tiene.',
    input_schema: {
      type: 'object',
      properties: {
