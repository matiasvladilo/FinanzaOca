...
const SUCURSALES: Sucursal[] = ['Todas', 'PV', 'La Reina', 'PT', 'Bilbao'];
...
export default function Header({ filters, onFiltersChange, onExport }: HeaderProps) {
  ...
  const setSucursal = (sucursal: Sucursal) => {
  ...
  return (
    <header className="flex items-center justify-between px-6 py-4 bg-white border-b border-gray-100 sticky top-0 z-30">
      ...
      <div className="flex items-center gap-3">
        ...
        {/* Date Range */}
        <button className="flex items-center gap-2 px-3 py-2 bg-white border border-gray-200 rounded-full text-[12px] text-gray-700 hover:border-blue-400 transition-colors">
          <Calendar className="w-3.5 h-3.5 text-gray-400" />
          <span>Jun 01 – Jun 30, 2024</span>
        ...
        <div className="relative">
          ...
          {sucursalOpen && (
            <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden z-50 min-w-[160px]">
              {SUCURSALES.map((s) => (
                <button
                  key={s}
